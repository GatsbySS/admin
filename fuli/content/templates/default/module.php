...
$sql = "SELECT gid,title,date,content,sortid,excerpt,views,comnum FROM ".DB_PREFIX."blog WHERE sortid=".$sort." AND hide='n' ORDER BY `date` DESC LIMIT 0,$num";
$go = $db->query($sql);while($row = $db->fetch_array($go)){
$img_url = TEMPLATE_URL.'images/sheli.jpg';//无图片时显示
if(picthumb($row['gid'])){$img_url = picthumb($row['gid']);//查找附件第一张图片
}else{$img_url;}?>
<a class="item" tapmode onclick="toBanner('<?php echo Url::log($row['gid']);?>')">
                <div class="item-left">
                    <img src="<?php echo $img_url;?>">
                </div>
                <div class="item-right">
                    <div class="item-title"><?php echo $row['title'];?></div>
                    <div class="item-subtitle"><?php echo $row['excerpt'];?></div>
