				...
				if( suxingme('suxingme_slide2',false) ): 
					for ($i=1; $i < suxingme('suxingme_slide_number') + 1; $i++) { ?>
							...
							<a href="<?php echo suxingme('suxingme_slide_url_'.$i);?>" title="<?php echo suxingme('suxingme_slide_title_'.$i);?>">
								<div class="slider-content" style="background: url(<?php echo suxingme('suxingme_slide_img_'.$i); ?>);">	
				...
				<?php else: ?>
					<?php 
							...
							while (have_posts()) : the_post();?>
								<div class="item">	
									<a href="<?php the_permalink(); ?>" title="<?php the_title();?>">
										<div class="slider-content" style="background: url(<?php if (get_post_meta($post->ID,"postthumb_value",true ) ){ echo get_post_meta($post->ID,"postthumb_value",true); } elseif( suxingme('suxingme_timthumb') ){ echo get_template_directory_uri().'/timthumb.php?src='.post_thumbnail_src().'&h=500&w=1120'; } else { echo post_thumbnail_src(); } ?>); ">
											<div class="slider-content-item">
						...
						<?php 
							...
							<div class="item" >
								<a href="<?php the_permalink(); ?>" title="<?php the_title();?>">
									<div class="slider-content" style="background: url(<?php if( suxingme('suxingme_timthumb') ){ echo get_template_directory_uri().'/timthumb.php?src='.post_thumbnail_src().'&h=450&w=750'; } else { echo post_thumbnail_src(); } ?>);">
										<div class="slider-content-item">
											<div class="slider-cat clearfix"><?php $category = get_the_category();if($category[0]){echo $category[0]->cat_name;}?></div>  
											<h2><?php the_title();?></h2>
							           	</div>
